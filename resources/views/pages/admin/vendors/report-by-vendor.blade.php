  ...
  <div class="row">
    <div class="col-md-12">
      <div class="filter-elements">
        <div class="filter-label">{!! trans('admin.vendor_name_label') !!}:</div>
        <div class="filter-input">
          <select name="vendor_name" id="vendor_name" class="select2" style="width:250px;"> 
            <option value="">{!! trans('admin.select_a_vendor') !!}</option>
            @if(count($all_vendor) > 0)
              @foreach($all_vendor as $vendor)
                @if($vendor->name == $vendor_name)
                <option selected="selected" value="{{ $vendor->name }}">{!! $vendor->display_name !!}</option>
                @else
                <option value="{{ $vendor->name }}">{!! $vendor->display_name !!}</option>
                @endif
              @endforeach
            @endif
          </select>  
        ...
        <table id="table_for_manufacturers_list" class="table table-bordered table-striped">
          ...
          <tbody>
            @if(!empty($vendor_reports_log) && $vendor_reports_log->count() > 0)
              ...
              @endforeach
            @else
              <tr><td colspan="6"><i class="fa fa-exclamation-triangle"></i> {!! trans('admin.no_data_found_label') !!}</td></tr>
            @endif
          </tbody>
          <tfoot>
            <th>{{ trans('admin.order_id') }}</th>
