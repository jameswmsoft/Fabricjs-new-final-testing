  ...
  <div class="box-body">
    <div class="row">
      <div class="col-md-4">
        ...
        {{--Priyanka--}}
        @if(!empty($order_data_by_id['_csv_filename']))
        <p><strong>{{ trans('Download Original Mailing List') }}:</strong> <a href='{!! url('/public/csv/'.$order_data_by_id['_csv_filename']) !!}'><button class="btn btn-primary btn-info">Download</button></a></p>
        {{--<p><strong>{{ trans('View CSV Data') }}:</strong> {!! $order_data_by_id['_csv_data'] !!}</p>--}}        
        <p><strong>{{ trans('View CSV Data') }}:</strong><br>       
        @foreach ($order_data_by_id_csv_data as $items)
          {{$items['first_name']}} 
          {{$items['last_name']}}
          {{$items['email']}}
          <br>
