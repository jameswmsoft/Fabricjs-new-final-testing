...
@section('content')
      ...
      <form class="form-horizontal" method="post" action="" enctype="multipart/form-data">
        ...
        <div class="col-md-10 col-sm-12 col-centered">    
          <div class="checkout-content">
                ...
                <ul class="cart-data">
                  ...
                  @foreach(Cart::items() as $index => $items)
                    <li class="row items-inline">
                      ...
                      <div class="price" data-price="{{$items->price}}">{!! price_html( get_product_price_html_by_filter($items->price) ) !!}</div>
                ...
                <div class="login-information clearfix">
                  <div class="login-content-sub">
                    ...
                    <div class="form_content">
                      ...
                      <!-- Start Joseph -->
                      <a class="btn btn-primary" href="{{ route('user-registration-page') }}" 
                        onclick="sessionStorage.setItem('checkout_active_step', 'login_user_address'); <?php session()->put('previous_url', 'checkout'); ?>">{!! trans('frontend.create_account_label') !!}</a>
                      <!-- End Joseph -->
                    </div>
                  </div>
                  <div class="login-content-sub">
                    ...
                    <div class="form_content">
                      ...
                      <!-- Start Joseph -->
                      <a class="btn btn-primary" href="{{ route('user-login-page') }}" onclick="sessionStorage.setItem('checkout_active_step', 'login_user_address');<?php session()->put('previous_url', 'checkout'); ?>">{!! trans('frontend.signin_account_label') !!}</a>
                      <!-- End Joseph -->
                    </div>
                  </div>
                </div>  
            ...
            @endif
            <!-- Start Joseph -->
            <button class="action next btn btn-primary">Continue</button>
  ...
  <!-- Start Joseph -->
  <script>
  $('.row').on('input', '.cart_quantity_input', function(e) {
      var unit_price = $(this).closest('.row').find('.price').data('price');
      ...
      if(quantity !== '' && quantity !== '0') {
          ...
          data.push({
              name: 'update_cart', value:  'update_cart'
          });
          $.post(window.location.href, data, function(res) {
              console.log(res)
          final_total = formatter.format( unit_price * parseInt(quantity) );
          var row = $(that).closest('.row');
          row.find('.last-total').html(final_total);
          $('.cart-sub-total .value').html(formatter.format(res.total));
          $('.cart-grand-total .value').html(formatter.format(res.total));
          })
          //$('.last-total').html(final_total);
      }
  });
  </script>
  <!-- End Joseph -->
