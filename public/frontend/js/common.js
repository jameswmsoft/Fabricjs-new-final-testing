...
$(document).ready(function(){
  ...
  // Init buttons and UI
  //Start Joseph
  var active_step = sessionStorage.getItem('checkout_active_step');

  if(active_step !== undefined && active_step !== null && $('#is_user_login').val() === '1') {
    widget.not(':eq('+((parseInt( $('#checkout_page .checkout-content').find('#' + active_step ).index() ) - 1))+')').hide();
    current = parseInt( $('#checkout_page .checkout-content').find('#payment').index() ) - 1;

    setTimeout(function() {
      localStorage.removeItem('checkout_active_step');
    }, 60000);
  } else {
  ...
  if($('.vendor-special-products-menu').length>0){
    $('#vendor_home_content ul.vendor-special-products-menu li a').on('click', function(){
      ...
      $(".special-items").slick("unslick");
      setTimeout(slickInit, 10);
    });
  }
});
