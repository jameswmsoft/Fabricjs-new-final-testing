...
var loadDesignFromJson = function( design_id ){
  ...
  if(selected_json_data){
    ...
    setExportCanvasDimension(parseJson.backgroundImage.width, parseJson.backgroundImage.height, function(status){
      if(status == 'done'){
        $('#custom_option_width').val( parseJson.backgroundImage.width );
        $('#custom_option_height').val( parseJson.backgroundImage.height );
        canvasWidth = parseJson.backgroundImage.width;
        canvasHeight = parseJson.backgroundImage.height;
...
var setExportCanvasDimension = function(width, height, callback){
  designerExportCanvasObj.setWidth( width );
  designerExportCanvasObj.setHeight( height );
  designerExportCanvasObj.renderAll();
