              ...
              if(get_product_type($cart_items->id) === 'customizable_product'){
                ...
                if($post->id)
                {
                  $usersCustomData->order_id           =   $post->id;
                    ...
                    //CSV data table update

                      $category = DB::select("SELECT MIN(`term_id`) AS term_id FROM `object_relationships` WHERE `object_id`= $items->id");
                      
                      foreach ($category as $category) {
                      if($category->term_id !='19') {

                      DB::update('UPDATE `csv_data` SET `order_id`= (SELECT `order_id` FROM `orders_items` ORDER BY `order_id` DESC LIMIT 1) ORDER BY id DESC LIMIT 1');
                      DB::update('UPDATE `orders_items` INNER JOIN `csv_data` ON orders_items.order_id=csv_data.order_id SET orders_items.csv_filename=csv_data.csv_filename');
                      DB::update('UPDATE `orders_items` INNER JOIN `csv_data` ON orders_items.order_id=csv_data.order_id SET orders_items.csv_data=csv_data.csv_data');
                                           
                      //End - CSV data table update
                      }
                    }
              ...
              if($cart_items->variation_id && count($cart_items->options) > 0){
                ...
                if($product_data['product_manage_stock'] == 'yes'){
                  ...
                  if($current_qty == 0){
                    ...
                    Product::where(['id' => $cart_items->id])->update($stock_ava);
                  }
                }
              }
            }
