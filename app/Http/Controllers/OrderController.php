...
class OrderController extends Controller
{
  ...
  public function orderDetailsPageContent( $params ){
    ...
    if($get_post_by_order_id->count() > 0 && $get_postmeta_by_order_id ->count() > 0 && $get_orders_items->count() >0){
      ...
      //$get_csv_filename = DB::select("SELECT `csv_filename` FROM `orders_items` WHERE `order_id`='$order_id'");
      //$order_data_by_id['_csv_filename'] = OrdersItem::select('csv_filename')->where(['order_id' => $order_id])->first();
      
      //Display csv filename and csv data in admin
      $order_data_by_id['_csv_filename'] = $get_orders_items->csv_filename;
      
      //$order_data_by_id['_csv_data'] = DB::select("SELECT `csv_data` FROM `csv_data` WHERE `order_id`='$order_id'");
      $order_data_by_id['_csv_data'] = $get_orders_items->csv_data;
      
      $order_data_by_id_csv_data = json_decode($get_orders_items->csv_data, true);

      //End - Display csv filename and csv data in admin
    ...
    //$csv_filename = DB::select("SELECT `csv_filename` FROM `orders_items` WHERE `order_id`='$order_id'");
    
    //Return CSV filename and csv data to admin order details page
    return view('pages.admin.orders.order-details', ['order_data_by_id_csv_data'=>$order_data_by_id_csv_data], $data);
  }
