...
class ProductsController extends Controller
{
  ...
  public function productAddContent(){
    ...
    //Removal of Front products from Multiple Back section on product details page
    
    DB::update('UPDATE `object_relationships` INNER JOIN `terms`
    ON object_relationships.term_id = terms.term_id
SET object_relationships.Pfb = terms.fb');

DB::update('UPDATE `products` INNER JOIN `object_relationships`
    ON products.id = object_relationships.object_id
SET products.Pfb = object_relationships.Pfb');

    //End - Removal of Front products from Multiple Back section on product details page
  ...
  public function productUpdateContent( $params ){
    ...
    //Removal of Front products from Multiple Back section 
...
SET products.Pfb = object_relationships.Pfb');
    ...
    //End - Removal of Front products from Multiple Back section 
        ...
        if(count($cat_term) > 0){
          ...
          if(count($get_cat_product_list) >0){
            foreach($get_cat_product_list as $product){
              //hide main products from multiple backs
              if(($product->id != $product_id ) && ($product->Pfb != '0' )){
              //End - hide main products from multiple backs
