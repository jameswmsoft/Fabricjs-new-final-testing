        ...
        if(!empty($data) && isset($data->password) && isset($data->id)){
          ...
          if(Hash::check(Input::get('login_password'), $password) && Hash::check(Input::get('login_password'), $data->password)){
            ...
            if($remember == TRUE){
              ...
              return $previous_url ? redirect($previous_url) : redirect()->route('user-account-page')->withCookie(cookie()->forever('frontend_remember_me_data', $cookieData));
            }
            elseif($remember == FALSE){
              if(Cookie::has('frontend_remember_me_data')){
                $cookie = Cookie::forget('frontend_remember_me_data');
                return $previous_url ? redirect($previous_url) : redirect()->route('user-account-page')->withCookie( $cookie );
              }
              else {
                return $previous_url ? redirect($previous_url) : redirect()->route('user-account-page');
              }
            }
            // End Joseph
          }
