...
class FrontendAjaxController extends Controller
{
  ...
  public function productAddToCart()
  {
    if(Request::isMethod('post') && Request::ajax() && Session::token() == Request::header('X-CSRF-TOKEN')){
      ...
      if(isset($input['variation_id']) && $input['variation_id']){
        $this->classCommonFunction->add_to_cart($input['product_id'], $input['qty'], $input['variation_id']);
      }
      else{
  ...
  public function saveCustomDesignImageForWatermark(){
    if(Request::isMethod('post')){
      if(Session::token() == Request::header('X-CSRF-TOKEN')){
        $input = Request::all();
        $destinationPath = public_path('uploads');
        $fileName = uniqid(time(), true). '.' .'png';
        $option = $this->option->getCustomDesignerSettingsData();
        $watermark_img = '';
        
        $upload_success = Input::file('design_img')->move($destinationPath, $fileName);
        
        if ($upload_success) {
          $original_img = url('/').'/public/uploads/'.$fileName;
          
          if(isset($option['watermark_img_url']) && !empty($option['watermark_img_url'])){
            $watermark_img = $option['watermark_img_url'];
          }
          
          if(!empty($original_img) && !empty($watermark_img)){
            $newFileName = uniqid(time(), true). '.' .'png';
            $path  = public_path('uploads/' . $newFileName);
            
            $img = Image::make( $original_img );
            $img->insert($watermark_img, 'center', 10, 10);
            $img->save($path);
            ...
            if(!Session::has('frontend_customer_img_download')){
              Session::put('frontend_customer_img_download', $newFileName);
            }
            ...
            if(!Session::has('frontend_customer_img_download')){
              Session::put('frontend_customer_img_download', $fileName);
            }
    ...
    if(Request::isMethod('post') && Request::ajax() && Session::token() == Request::header('X-CSRF-TOKEN')){
      ...
      if(isset($input['customizeData'])){
        ...
        if(Cache::add($input['accessToken'], $input['customizeData'], $expiresAt)){
          if(isset($input['variation_id']) && $input['variation_id']){
            $this->classCommonFunction->add_to_cart($input['product_id'], $input['qty'], $input['variation_id'], $input['accessToken'], $input['customizeData']);
          }
          else{
            $this->classCommonFunction->add_to_cart($input['product_id'], $input['qty'], 0, $input['accessToken'], $input['customizeData']);
          }
        }
      }
    }
  ...
  */
  ...
  public function forceDesignerImageFile(){
    $file_name = '';
    if(Session::has('frontend_customer_img_download')){
      $file_name = Session::get('frontend_customer_img_download');
      Session::forget('frontend_customer_img_download');
    }
