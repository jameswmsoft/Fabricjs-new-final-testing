  ...
  public function designerSinglePageContents($param1,$param2){
    ...
    if(!empty($get_product)){
      ...
      if($get_product_type == 'customizable_product'){
        ...
        $category = DB::select("SELECT MIN(`term_id`) AS term_id FROM `object_relationships` WHERE `object_id`= '$product_id'");        
        
        return view('pages.frontend.frontend-pages.frontend-designer', ['category'=>$category], $data);
